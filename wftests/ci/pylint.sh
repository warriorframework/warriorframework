...
if [[ $(git --no-pager diff --name-only develop "${TRAVIS_COMMIT}"  | grep -v 'OSS' | grep '.py$') ]]; then
    ...
    git --no-pager diff --name-only develop "${TRAVIS_COMMIT}"  | grep -v 'OSS' | grep '.py$'
else
...
git --no-pager diff --name-only develop "${TRAVIS_COMMIT}"  | grep -v 'OSS' | grep '.py$' | xargs -L 1 pylint || true
git checkout "${TRAVIS_COMMIT}" ;
git --no-pager diff --name-only "${TRAVIS_COMMIT}" develop  | grep -v 'OSS' | grep '.py$'
git --no-pager diff --name-only "${TRAVIS_COMMIT}" develop  | grep -v 'OSS' | grep '.py$' | xargs -L 1 pylint | tee pylint_result.txt || true
