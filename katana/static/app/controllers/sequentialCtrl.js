...
var printFormater = {
	...
	sortingAPI: {
		...
		filterBar: function( filterList, filterScope ){
			if( printFormater.bar.siblings('.filterBar').length != 0 ){
				...
				for( var i = 0; filterList.length > i; i++ ){
					...
					filter.find('input').on( 'change', function(){printFormater.sortingAPI.filter( bar );} );
				}
				bar.appendTo('.nav');
		...
		filter: function( bar ){
			if( bar ){
				var activeFilters = bar.find('input:checked').map(function () { return $(this).val(); }).get();
	...
	getHtml: function(){
		$.ajax({
			...
			if( content != '' ){
				...
				setTimeout(function(){ executeApi.getHtml(); } , 400);
		});
	},
