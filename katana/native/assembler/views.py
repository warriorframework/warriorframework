...
class AssemblerView(View):
    ...
    def get(self, request):
        nav_obj = Navigator()
        empty_template = os.path.join(nav_obj.get_katana_dir(), "native", "assembler", "static",
        ...
        if not isinstance(json_data["data"]["drivers"]["repository"], list):
            json_data["data"]["drivers"]["repository"] = [json_data["data"]["drivers"]["repository"]]
        if not isinstance(json_data["data"]["warriorspace"]["repository"], list):
            json_data["data"]["warriorspace"]["repository"] = [json_data["data"]["warriorspace"]["repository"]]
        return render(request, 'assembler/assembler.html', {"xml_contents": json_data})