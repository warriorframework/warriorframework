...
def execute_steps(step_list, data_repository, system_name, parallel, queue):
    ...
    while step_num < len(step_list):
        step_num, kw_resultfile_list, data_repository, step_status_list, step_impact_list, do_continue = _execute_step(step_list, step_num, goto_stepnum, kw_resultfile_list, data_repository, default_error_action, default_error_value, step_status_list, step_impact_list, system_name)
...
def _execute_step(step_list, step_num, goto_stepnum, kw_resultfile_list, data_repository,
                  default_error_action, default_error_value, step_status_list, step_impact_list, system_name, skip_recovery=True):
    ...
    if not goto_stepnum or goto_stepnum == str(step_num):
        ...
        if not run_current_step:
            ...
            if trigger_action.upper() in ['ABORT', 'ABORT_AS_ERROR']:
                return step_num, kw_resultfile_list, data_repository, step_status_list, step_impact_list, "break"
            elif trigger_action.upper() in ['SKIP', 'NEXT']:
                return step_num, kw_resultfile_list, data_repository, step_status_list, step_impact_list, "continue"
            ...
            # change the next iteration point to goto value
                ...
                for goto_step in goto_stepnum:
                    if int(goto_step) - 1 < len(step_list):
                        _, kw_resultfile_list, data_repository, step_status_list, step_impact_list, _ = _execute_step(step_list, int(goto_step) - 1, False, kw_resultfile_list, data_repository, default_error_action, default_error_value, step_status_list, step_impact_list, system_name, skip_recovery=False)
