...
def get_system_console_log(filename, logsdir, console_name):
    ...
    print_info("************ This is parallel execution... console logs for {0} will be "
                "logged in {1} ************".format(console_name, console_logfile))
...
def execute_steps(step_list, data_repository, system_name, parallel, queue):
    ...
    for x in xrange(progress_bar.progress.total):
        while step_num < len(step_list):
            ...
            if not goto_stepnum:
                ...
                data_repository['wt_junit_object'].\
                    add_keyword_result(data_repository['wt_tc_timestamp'], step_num, keyword,
