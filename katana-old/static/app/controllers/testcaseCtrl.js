...
app.controller('TestcaseCtrl', ['$scope', '$http', 'fileFactory', function ($scope, $http, fileFactory) {
                                            ...
                                            return false
                                              ...
                                              if(inputValue.indexOf("/") !== -1){
                                                ...
                                                fileFactory.checkfilepath(test_value)
                                                    ...
                                                    function(data) {
                                                        if(data["exists"] == "yes"){
                                                            fileFactory.updateconfigfromtab("xmldir", test_value)
                                                            .then(
                                                                function(data) {
                                                                    if(data["updated"] == "yes") {
                                                                        readConfig();
    ...
    function readTestCaseFileNames(directory) {
        $http.get('/testcasefilenames/' + directory)
            .success(function (data, status, headers, config) {
    ...
    function readTestCaseFolderNames(directory) {
        $http.get('/testcasefoldernames/' + directory)
            .success(function(data, status, headers, config) {
                $scope.xml.folders = data;
                console.log('testcasefoldernames: ' + JSON.stringify(data));
            })
            .error(function(data, status, headers, config) {
